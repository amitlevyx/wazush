...
def combine_test_rows(data):
    data1, data2, data3, data4 = data[data.index % 4 == 0], data[data.index
                                                                 % 4 == 1], \
                                 data[data.index % 4 == 2], data[
                                     data.index % 4 == 3]
    dfs = [data1, data2, data3, data4]
    for i in range(len(dfs)):
        dfs[i] = dfs[i].add_prefix('event_' + str(i + 1) + '_')
...
def preprocess_test_task_1(data: pd.DataFrame, train_data: pd.DataFrame):
    data = data.drop_duplicates(subset=['OBJECTID'])
    data = data.loc[data['linqmap_city'] == 'תל אביב - יפו']
    data['linqmap_subtype'] = impute_subtypes(data[['linqmap_type',
                                                    'linqmap_subtype']])
    ...
    # split to four events and fifth one
    data = combine_test_rows(data)

    # distance between two points
    data = add_dist_to_data(data)
    data = data.reindex(columns=train_data.columns, fill_value=0)
    return data
